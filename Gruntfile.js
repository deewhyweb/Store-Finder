...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    bgShell: {
      ...
      clienttest: { // run karma for client tests
        cmd: 'cd client/default;karma start karmaConf.js --single-run'
      },
      clienttest2: { // run karma for client tests, keep running
        cmd: 'cd client/default;karma start karmaConf.js'
      },
      cloudtest: { // run mocha tests for cloud code
        cmd : 'cd cloud;npm test'
      }
    }
  });
  ...
  grunt.registerMultiTask('build', 'Clear and copy the files to relevant dirs for Cordova development', function() {
    ...
    switch (data.type) {
      ...
      default:
        // Expecting just 'packages', so let's do that
        copyConfig = _iteratePackages(data.packages, null, self, copyConfig);
    }
  ...
  grunt.registerTask('clienttest', ['bgShell:clienttest']);
  grunt.registerTask('clienttest2', ['bgShell:clienttest2']);
  grunt.registerTask('cloudtest', ['bgShell:cloudtest']);
